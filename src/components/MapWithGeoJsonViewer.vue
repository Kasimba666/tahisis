    ...
    <div class="map-geojson-viewer">
      ...
      <div class="viewer-controls">
        ...
        <div class="view-controls">
          <el-space>
            <el-radio-group v-model="mapProvider" size="small" @change="onProviderChange">
      ...
      <div class="content-area" :class="{ 'split-view': currentView === 'split' }">
        ...
        <div v-show="currentView === 'map' || currentView === 'split'" class="map-container">
          ...
          <div v-show="mapProvider === 'leaflet'" ref="mapContainer" class="map-wrapper"></div>
          <div v-show="mapProvider === 'openlayers'" ref="olMapContainer" class="map-wrapper"></div>
        </div>
...
import { mapView } from '@/store/mapView.js'
...
export default {
  ...
  methods: {
    // apply view from shared store (center/zoom/bounds)
    setViewFromStore() {
      try {
        if (!this.map) return
        if (mapView && mapView.bounds && Array.isArray(mapView.bounds) && mapView.bounds.length === 2) {
          const b = mapView.bounds
          this.map.fitBounds([[b[0][0], b[0][1]], [b[1][0], b[1][1]]])
        } else if (mapView && mapView.center && mapView.zoom) {
          this.map.setView([mapView.center.lat, mapView.center.lon], mapView.zoom)
        }
      } catch (e) { /* noop */ }
    },
    ...
    open(settlementsData) {
      ...
      try { if (mapView && mapView.provider) { this.mapProvider = mapView.provider } } catch (e) {}
    },

    // initialize OpenLayers map and fit view
    initializeOlMap() {
      if (!this.$refs.olMapContainer) return
        ...
        if (!this.olMap) {
          this.olVectorLayer = new VectorLayer({
            ...
            style: () => new Style({
              image: new Circle({
                radius: 6,
                fill: new Fill({ color: 'hsl(0, 0%, 30%)' }),
          ...
          this.olMap = new OLMap({
            target: this.$refs.olMapContainer,
            layers: [
          ...
          this.$refs.olMapContainer.appendChild(this.olPopupEl)
          ...
          this.olMap.addOverlay(this.olPopupOverlay)

          this.olMap.on('singleclick', (evt) => {
            let shown = false
            this.olMap.forEachFeatureAtPixel(evt.pixel, (feature) => {
              const props = feature.get('props') || {}
              const nameOld = props.name_old || props.name_modern || ''
              const nameModern = props.name_modern || ''
              const district = props.district_name || ''
              const population = (props.population && props.population.total) ? props.population.total : (props.population || '')

              let html = `<div class="map-popup">`
              html += `<h4>${nameOld}</h4>`
              if (nameModern && nameModern !== nameOld) {
                html += `<p><strong>Современное:</strong> ${nameModern}</p>`
              }
              if (district) {
                html += `<p><strong>Уезд:</strong> ${district}</p>`
              }
              if (population) {
                html += `<p><strong>Население:</strong> ${population}</p>`
              }

              if (Array.isArray(props.estate_types) && props.estate_types.length) {
                html += `<div class="estate-types"><strong>Сословия:</strong>`
                html += props.estate_types.map(t => `<div>• ${t.type_name}: ${t.total_count}</div>`).join('')
                html += `</div>`
              }
              ...
              if (Array.isArray(props.subtypes) && props.subtypes.length) {
                extraParts.push(`<p><strong>Подтипы:</strong> ${props.subtypes.join(', ')}</p>`)
              }
              if (Array.isArray(props.religions) && props.religions.length) {
                extraParts.push(`<p><strong>Конфессии:</strong> ${props.religions.join(', ')}</p>`)
              }
          ...
          this.olMap.on('pointermove', (evt) => {
            ...
            try { this.$refs.olMapContainer.style.cursor = hit ? 'pointer' : 'default' } catch(e) {}
          })
        }

        // apply main map view if available
        try {
          if (mapView && mapView.bounds && Array.isArray(mapView.bounds) && mapView.bounds.length === 2) {
            const b = mapView.bounds
            const extent4326 = [b[0][1], b[0][0], b[1][1], b[1][0]]
            const extent3857 = transformExtent(extent4326, 'EPSG:4326', 'EPSG:3857')
            this.olMap.getView().fit(extent3857, { duration: 0 })
          } else if (mapView && mapView.center && mapView.zoom) {
            this.olMap.getView().setCenter(fromLonLat([mapView.center.lon, mapView.center.lat]))
            this.olMap.getView().setZoom(mapView.zoom)
          }
        } catch (e) {}

        // add current features
        this.addPointsToOlMap()
        this.olInitialized = true
        setTimeout(() => { try { this.olMap.updateSize() } catch(e) {} }, 50)
      } catch (error) {
        console.error('Error initializing OpenLayers map:', error)
        ElMessage.error('Не удалось инициализировать карту OpenLayers')
      }
    },

    // add features to OpenLayers map
    addPointsToOlMap() {
      if (!this.olVectorLayer || !this.currentData || !this.currentData.features) return
      try {
        const source = this.olVectorLayer.getSource()
        source.clear()
        this.currentData.features.forEach(f => {
          if (!f.geometry || !f.geometry.coordinates) return
          const [lng, lat] = f.geometry.coordinates
          const feat = new Feature({ geometry: new Point(fromLonLat([lng, lat])) })
          try { feat.set('props', f.properties || {}) } catch(e) {}
