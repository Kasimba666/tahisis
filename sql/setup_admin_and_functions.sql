-- =====================================================
-- ФИНАЛЬНЫЙ СКРИПТ — ВЫПОЛНИТЕ ОДИН РАЗ
-- =====================================================

-- =====================================================
-- Функция 1: Создание заявки на регистрацию
-- Теперь не использует FK к auth.users, а создаёт профиль
-- напрямую, дожидаясь готовности auth.users
-- =====================================================
...
DECLARE
  ...
  v_attempt int := 0;
BEGIN
  -- Пытаемся вставить с повторными попытками (ждём пока auth.users создастся)
  LOOP
    BEGIN
      ...
      EXIT; -- Успешно, выходим из цикла
    EXCEPTION WHEN foreign_key_violation THEN
      v_attempt := v_attempt + 1;
      IF v_attempt >= 10 THEN
        RETURN jsonb_build_object('success', false, 'error', 'Таймаут ожидания создания пользователя');
      END IF;
      PERFORM pg_sleep(0.3); -- ждём 300 мс
    END;
  END LOOP;
...
-- =====================================================
-- Функция 2: Одобрение заявки (с подтверждением email)
-- =====================================================
...
BEGIN
  ...
  -- Подтверждаем email пользователя (чтобы мог войти без подтверждения)
  UPDATE auth.users
  SET email_confirmed_at = COALESCE(email_confirmed_at, now())
  WHERE id = v_request.user_id;

  RETURN jsonb_build_object('success', true, 'user_id', v_request.user_id, 'role', v_request.requested_role);
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object('success', false, 'error', SQLERRM);
END;
$$;
...
-- =====================================================
-- Функция 3: Удаление пользователя (для админки)
-- =====================================================
...
-- =====================================================
-- Подтверждаем email всех существующих пользователей
-- =====================================================
